...
func TestEndToEndDeploymentScenario(t *testing.T) {
	...
	test_structure.RunTestStage(t, "validate", func() {
	    ...
	    // test ssh connect
	    vmLinuxPublicIPAddress := terraform.Output(t, terraformOptions, "vm_linux_public_ip_address_1")
	    ...
	    dbhost := terraform.Output(t, terraformOptions, "database_host_fqdn")
	    pass := os.Getenv("passdb")
	    testdb := "mysql -u user -h " + dbhost + " -D db --password=" + pass + " -e 'SELECT VERSION();'"

            err = sshSession.Run(testdb)
            if err != nil {
