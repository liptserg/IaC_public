resource "yandex_lb_target_group" "wp_tg" {
  ...
  dynamic "target" {
    ...
    content {
      subnet_id  = yandex_compute_instance.wp-app[target.key].network_interface.0.subnet_id
      address    = yandex_compute_instance.wp-app[target.key].network_interface.0.ip_address
    }
  }
