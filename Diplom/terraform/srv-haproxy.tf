resource "yandex_compute_instance" "srv-haproxy35" {
  name = "srv-haproxy"
  zone = var.zones[0]
  ...
  provisioner "remote-exec" {
    inline = [     
      ...
      "sudo hostnamectl set-hostname srv-haproxy",
      "sudo sed -i 's/127.0.0.1[[:space:]].*/127.0.0.1 localhost srv-haproxy/g' /etc/hosts",
